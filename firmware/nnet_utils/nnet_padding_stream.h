...
template<class res_T, typename CONFIG_T>
void fill_zero_s(hls::stream<res_T> &res) {
    #pragma HLS INLINE
    res_T res_part;
    for (int c = 0; c < CONFIG_T::n_chan; c++) {
        #pragma HLS UNROLL
...
template<class res_T, typename CONFIG_T>
void fill_zero_ss(hls::stream<res_T> res[1]) {
    #pragma HLS INLINE
...
template<class data_T, class res_T, typename CONFIG_T>
void fill_data_s(hls::stream<data_T> &data, hls::stream<res_T> &res) {
    #pragma HLS INLINE
...
template<class data_T, class res_T, typename CONFIG_T>
void fill_data_ss(hls::stream<data_T> data[1], hls::stream<res_T> res[1]) {
    #pragma HLS INLINE
...
void zeropad2d_cl_s(
    ...
    PadTop: for (int i = 0; i < CONFIG_T::pad_top; i++) {
        PadTopWidth: for (int j = 0; j < CONFIG_T::out_width; j++) {
            fill_zero_s<res_T, CONFIG_T>(res);
        }
    }
    ...
    PadMain: for (int i = 0; i < CONFIG_T::in_height; i++) {
        ...
        CopyMain: for (int j = 0; j < CONFIG_T::in_width; j++) {
            fill_data_s<data_T, res_T, CONFIG_T>(data, res);
        }
        PadRight: for (int j = 0; j < CONFIG_T::pad_right; j++) {
            fill_zero_s<res_T, CONFIG_T>(res);
        }
