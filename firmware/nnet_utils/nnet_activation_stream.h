...
template<class data_T, class res_T, typename CONFIG_T>
void relu_ss(hls::stream<data_T> data[1], hls::stream<res_T> res[1]) {
...
template<class data_T, class res_T, typename CONFIG_T>
void leaky_relu_s(hls::stream<data_T> &data, data_T alpha, hls::stream<res_T> &res) {
    LeakyReLUActLoop: for (int i = 0; i < CONFIG_T::n_in / res_T::size; i++) {
        #pragma HLS PIPELINE
        ...
        LeakyReLUPackLoop: for (int j = 0; j < res_T::size; j++) {
            #pragma HLS UNROLL
			data_T in_data = data.read();
            if (in_data > 0) out_data[j] = in_data;
            else out_data[j] = alpha * in_data;
        }
        res.write(out_data);
    }
}
...
template<class data_T, class res_T, typename CONFIG_T>
void leaky_relu_ss(hls::stream<data_T> data[1], data_T alpha, hls::stream<res_T> res[1]) {
