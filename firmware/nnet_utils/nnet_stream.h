...
template<class data_T, class res_T, int N>
void clone_stream_s(hls::stream<data_T> &data, hls::stream<res_T> &res1, hls::stream<res_T> &res2) {
    CloneLoop: for (int i = 0; i < N / 992; i++) {
        #pragma HLS PIPELINE
		data_T in_data[992];
        for(int j = 0;j < 992;j++){
            in_data[j] = data.read();
...
template<class data_T, class res_T, int N>
void clone_stream_ss(hls::stream<data_T> data[1], hls::stream<res_T> res1[1], hls::stream<res_T> res2[1]) {
...
void clone_stream_single(
    hls::stream<data_T> data[1],
    hls::stream<res_T> res1[1],
    hls::stream<res_T> res2[1] ) {

    CloneLoop: for (int i = 0; i < CONFIG_T::n_in; i++) {
        #pragma HLS PIPELINE II=1
...
template<class data_T, class res_T, typename CONFIG_T>
void clone_stream_array(
    hls::stream<data_T> data[CONFIG_T::n_chan],
    hls::stream<res_T> res1[CONFIG_T::n_chan],
    hls::stream<res_T> res2[CONFIG_T::n_chan] ) {

    CloneLoop: for (int i = 0; i < CONFIG_T::n_in / CONFIG_T::n_chan ; i++) {
        #pragma HLS PIPELINE II=1

        res_T in_data[CONFIG_T::n_chan];
        #pragma HLS ARRAY_PARTITION variable=in_data complete

        Input: for (int j = 0; j < CONFIG_T::n_chan; j++) {
            #pragma HLS UNROLL
            in_data[j] = data[j].read();
        }

        Output: for (int j = 0; j < CONFIG_T::n_chan; j++) {
            #pragma HLS UNROLL
            res_T out_data1 = in_data[j];
            res_T out_data2 = in_data[j];
...
template<class data_T, class res_T, typename CONFIG_T>
void clone_stream_switch(
    hls::stream<data_T> data[CONFIG_T::data_transfer_out],
    hls::stream<res_T> res1[CONFIG_T::data_transfer_out],
    hls::stream<res_T> res2[CONFIG_T::data_transfer_out]) {

    #pragma HLS inline region
    if(CONFIG_T::data_transfer_out == 1){
        clone_stream_single<data_T, res_T, CONFIG_T>(data, res1, res2);
    }else {
        clone_stream_array<data_T, res_T, CONFIG_T>(data, res1, res2);
    }
}
