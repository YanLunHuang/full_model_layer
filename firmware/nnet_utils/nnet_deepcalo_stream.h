...
template<class data_T, class res_T, typename CONFIG_T>
void sum1d_array(
        hls::stream<data_T> data[CONFIG_T::width],
        hls::stream<res_T> res[CONFIG_T::width]) {
    ...
    hls::stream<res_T> res[CONFIG_T::data_transfer_out]) 
{
    #pragma HLS inline region
    if(CONFIG_T::data_transfer_out == 1){
        sum1d_single<data_T, res_T, CONFIG_T>(data, res);
    }else {
        sum1d_array<data_T, res_T, CONFIG_T>(data, res);
    }
}
