    ...
    typename CONFIG_T::bias_t   biases[CONFIG_T::n_filt])
{
    ...
    static ap_shift_reg<data_T, CONFIG_T::in_width> line_buffer[MAX(CONFIG_T::filt_height - 1,1)][CONFIG_T::n_chan];
    #pragma HLS ARRAY_PARTITION variable = line_buffer complete dim = 2
    ...
    ReadInputHeight: for (unsigned i_ih = 0; i_ih < CONFIG_T::in_height; i_ih++) {
        ReadInputWidth: for (unsigned i_iw = 0; i_iw < CONFIG_T::in_width; i_iw++) {
            ...
            if (CONFIG_T::filt_height > 1) {
                compute_output_buffer_2d_ss<data_T, res_T, CONFIG_T>(data, line_buffer, res, weights, biases);
            } else {
                compute_output_buffer_2d_ss<data_T, res_T, CONFIG_T>(data, line_buffer, res, weights, biases);
            }
        }
