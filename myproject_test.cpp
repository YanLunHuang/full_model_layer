...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      hls::stream<input29_t> em_barrel[128];
      hls::stream<input13_t> scalars[128];
	  std::cout << "Input1" << std::endl;
      nnet::copy_data_switch<float, input29_t, 0, 7296, 128>(in, em_barrel);
	  std::cout << "Input2" << std::endl;
      nnet::copy_data_switch<float, input13_t, 7296, 7296, 128>(in, scalars);
      hls::stream<result_t> layer102_out[128];
      ...
      if (e % CHECKPOINT == 0) {
        ...
        nnet::print_result_switch<result_t, 7296, 128>(layer102_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result_switch<result_t, 7296, 128>(layer102_out, fout);
