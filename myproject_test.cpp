...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      hls::stream<input_t> tracks[128];
      nnet::copy_data_switch<float, input_t, 0, 7296, 128>(in, tracks);
      ...
      if (e % CHECKPOINT == 0) {
        ...
        nnet::print_result_switch<layer12_t, 128, 1>(layer12_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result_switch<layer12_t, 128, 1>(layer12_out, fout);
