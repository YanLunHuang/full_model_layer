...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      hls::stream<input29_t> em_barrel[16];
      nnet::copy_data_switch<float, input29_t, 0, 49280, 16>(in, em_barrel);
      hls::stream<input13_t> scalars[1];
      nnet::copy_data_switch<float, input13_t, 49280, 32, 1>(in, scalars);
      hls::stream<layer42_t> layer102_out[16];
      ...
      if (e % CHECKPOINT == 0) {
        ...
        for(int i = 0; i < 49280; i++) {
          std::cout << pr[i] << " ";
        }
        std::cout << std::endl;
        std::cout << "Quantized predictions" << std::endl;
        //hls-fpga-machine-learning insert quantized
        nnet::print_result_switch<layer42_t, 49280, 16>(layer102_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result_switch<layer42_t, 49280, 16>(layer102_out, fout);
