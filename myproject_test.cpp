...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      hls::stream<input_t> tracks[1];
      nnet::copy_data_switch<float, input_t, 0, 992, 1>(in, tracks);
      hls::stream<layer41_t> layer102_out[1];
      ...
      if (e % CHECKPOINT == 0) {
        ...
        nnet::print_result_switch<layer41_t, 32, 1>(layer102_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result_switch<layer41_t, 32, 1>(layer102_out, fout);
