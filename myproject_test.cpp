...
int main(int argc, char **argv)
{
  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      hls::stream<layer22_t> em_barrel[1];
      nnet::copy_data_switch<float, layer22_t, 0, 256, 1>(in, em_barrel);
      hls::stream<layer23_t> scalars[1];
      nnet::copy_data_switch<float, layer23_t, 256, 128, 1>(in, scalars);
      hls::stream<layer24_t> layer102_out[384];
      ...
      if (e % CHECKPOINT == 0) {
        ...
        nnet::print_result_switch<layer24_t, 384, 384>(layer102_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result_switch<layer24_t, 384, 384>(layer102_out, fout);
